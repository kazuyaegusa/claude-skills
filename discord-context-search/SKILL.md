...
- **キーワード**（任意）: 検索する単語。省略された場合は `AskUserQuestion` でユーザーに入力を求める
...
**引数にキーワードが含まれているか確認する。含まれていない場合（引数が空・なし）は、必ず最初に `AskUserQuestion` ツールでキーワードをユーザーに聞く。**

`AskUserQuestion` の設定:
- question: "検索するキーワードを入力してください（人名、プロジェクト名、トピック等）"
- header: "キーワード"
- options に「Other」が自動付与されるので、自由入力で受け取る形にする。よく使いそうな候補を2-3個 options に入れてもよい。

**キーワードを取得するまで Phase 1 以降に進んではならない。**
